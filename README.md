...
* **Configurable Journey Zones:** The variable journey_zones defines the maximum passenger capacity for the dispatch engine. During standard random flights, the system dynamically generates a passenger count between 0 and journey_zones. However, when an active journey is initialized via the -j pipeline, the passenger manifest is locked to the full journey_zones value for every leg of the expedition to ensure a stable, consistent payload throughout the tour. Users are free to adjust this value in their configuration to suit the scale of their chosen aircraft.
